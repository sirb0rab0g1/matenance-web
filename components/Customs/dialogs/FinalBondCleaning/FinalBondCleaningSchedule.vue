<template>
  <div>
    <div
      ...
      @click="$modal.show('finalbondcleaning')"
      style="margin-top: -10px;">
      <v-icon style="font-size: 36px;">arrow_drop_down</v-icon>
    </div>
    <modal
      name="finalbondcleaning"
      height="auto"
      :width="320"
      :clickToClose="false">
      <v-layout row wrap class="mb-2">
        ...
        <v-card class="pa-3 mt-2">
          Hours Estimate: {{ form.years === null ? ' ' : round(floor2((((parseFloat(form.venetian) * 30) + (parseFloat(form.rooms) * 60) + (parseFloat(form.kitchens) * 120)) * major) / 60)) }}
        </v-card>
        </v-flex>
        ...
        <v-flex xs12>
          ...
          <v-btn
            ...
            @click="$modal.hide('finalbondcleaning')"
          >
