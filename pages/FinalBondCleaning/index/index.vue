  ...
  <v-layout row wrap>
    <v-flex lg12>
      ...
      <v-flex xs12 v-if="location.postCode.length > 3" style="margin-top: -10px;">
        <div class="pl-1" style="margin-top: 10px;">There are 3 bond cleaning suppliers around you. Email me whenever another supplier is added.</div>
      </v-flex>
  ...
  export default {
    beforeRouteLeave (to, from, next) {
      ...
      if (this.location.postCode === '' || this.location.address === '') { // if back button
        ...
        this.$set(this.location, 'type', 'bondcleaning')
    ...
    methods: {
      ...
      setForm () {
        this.location = Object.assign({}, this.location, this.bond)
      }
    },
    transition: 'slide'
  }
