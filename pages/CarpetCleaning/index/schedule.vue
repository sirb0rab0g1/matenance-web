  ...
  <v-container style="color: #424242;">
    <v-layout row wrap>
      ...
      <v-flex xs12 style="margin-top: -20px;">
        <v-radio-group class="pasmo mb-negative-15" v-model="rooms" row :mandatory="false">

        <v-radio color="light-blue darken-3" :value="true">
          <div slot="label">
        ...
        <v-radio color="light-blue darken-3" :value="false">
          <div slot="label">
            <span style="color: #424242;" class="helvetica">
              Price by m2
            </span>
      ...
      <v-flex>
        <c-global field="textfield" :placeholder="rooms === true ? 'Number of Rooms' : 'Number of m2'" v-model="schedule.hours"/>
  ...
  export default {
    beforeRouteLeave (to, from, next) {
      ...
      if (this.schedule.hours === '' && this.schedule.frequency === '' && this.schedule.days.length <= 0 && this.schedule.startTime === '' && this.schedule.endTime === '' && this.schedule.date === '') {
        ...
        this.$set(this.schedule, 'type', 'carpetcleaning')
    ...
    methods: {
      ...
      setForm () {
        this.schedule = Object.assign({}, this.schedule, this.carpet)
      }
    },
    transition: 'slide'
  }
